...
| T3 | Public tunnel and Linq subscription | Claude | Blocked | No tunnel binary installed; `LINQ_API_KEY` / `LINQ_WEBHOOK_SECRET` unset in the environment | Needs the operator: set the two Linq variables, approve exposing the local port, then subscribe only to `message.received` at `/webhooks/linq?version=2026-02-03`. |
