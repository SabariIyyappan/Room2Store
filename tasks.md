...
| T7 | Real price: comps + Terac study | **Unclaimed — Engineer A** | Pending | `compsQuery` is emitted and ready to consume; `naivePrice.status` is `needs_comps` | `plan.md` A5–A8. The listing draft's `$25` is hard-coded. This is the project thesis and nothing else replaces it. |
| T8 | Compliance veto on the iMessage path | **Unclaimed — B rules, C call site** | Pending | `services/compliance/src/verdict.mjs` exists with 6 passing tests | `reviewItem`/`canDeploy` run only on the web confirm endpoint. A prohibited item sent by text still gets a listing draft, so the demo's car-seat veto would not fire. |
| T9 | Attach the photo to the listing draft | **Unclaimed — Engineer C** | Pending | Draft is text-only in `formatListingDraft` | Outbound media parts. Note the sandbox rule: the first outbound message in a new chat may not contain links or effects. |
| T10 | Persistence behind the database | **Unclaimed — Engineer B** | Pending | `services/perception/src/sessions.mjs` is an in-memory Map | `plan.md` B2. A redeploy or a free-tier spin-down wipes every session and item list. |

## File ownership

- **Released.** Claude's claim on `services/perception/**` and `services/compliance/**` is lifted as of 2026-08-15; anyone may edit them.
- **`services/compliance/` was written by Claude but belongs to Engineer B** under `plan.md` §1.3. Treat `verdict.mjs` as a starting point, not a claim — read it before writing a competing implementation.
